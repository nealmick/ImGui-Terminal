...
setup_vulkan(ImVector<const char *> instance_exts)
{
	...
	/* Instance + portability extensions for MoltenVK on macOS. */
	{
		...
		props.resize(pcount);
		err = vkEnumerateInstanceExtensionProperties(nullptr, &pcount, props.Data);
		check_vk(err);
...
setup_window(ImGui_ImplVulkanH_Window *wd, VkSurfaceKHR surface, int w, int h)
{
	...
	wd->SurfaceFormat = ImGui_ImplVulkanH_SelectSurfaceFormat(
	    g_physical, surface, fmts, IM_ARRAYSIZE(fmts), VK_COLORSPACE_SRGB_NONLINEAR_KHR);
	...
	ImGui_ImplVulkanH_CreateOrResizeWindow(g_instance, g_physical, g_device, wd, g_queue_family,
	    g_alloc, w, h, g_min_image_count, 0);
}
...
frame_render(ImGui_ImplVulkanH_Window *wd, ImDrawData *draw_data)
{
	...
	VkResult err = vkAcquireNextImageKHR(
	    g_device, wd->Swapchain, UINT64_MAX, acq, VK_NULL_HANDLE, &wd->FrameIndex);
...
main(int, char **)
{
	...
	GLFWwindow *window = glfwCreateWindow(800, 600, "st-imgui (vulkan)", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (cur_w > 0 && cur_h > 0 &&
		    (g_swapchain_rebuild || g_window_data.Width != cur_w ||
			g_window_data.Height != cur_h))
		{
			ImGui_ImplVulkan_SetMinImageCount(g_min_image_count);
			ImGui_ImplVulkanH_CreateOrResizeWindow(g_instance, g_physical, g_device, wd,
			    g_queue_family, g_alloc, cur_w, cur_h, g_min_image_count, 0);
		...
		bool minimized = (dd->DisplaySize.x <= 0.0f || dd->DisplaySize.y <= 0.0f);
